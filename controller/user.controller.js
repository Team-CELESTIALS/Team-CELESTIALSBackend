...
export const signup = async (req, res) => {
    try {
        const { fullname, email, password, registrationNumber, userImage, isAdmin } = req.body;
        ...
        const createdUser = new User({
            ...
            registrationNumber,
            userImage: userImage || '',
            isAdmin: isAdmin || false, // Set isAdmin based on input
        });
...
export const bulkUpdateUsers = async (req, res) => {
    try {
        const { users } = req.body; // Expecting an array of user objects with _id and fields to update

        const updatePromises = users.map(async (userData) => {
            const { _id, ...updates } = userData;
...
export const bulkDeleteUsers = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Users deleted successfully", result: deletedUsers });
    } catch (error) {
        console.error("Error: " + error.message);
        res.status(500).json({ message: "Internal server error" });
    }
